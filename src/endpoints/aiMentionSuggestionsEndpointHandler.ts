...
export const aiMentionSuggestionsEndpointHandler: PayloadHandler = async (req) => {
    ...
    if (!query || collectionMatches >= 3) return Response.json({ suggestions: [] })

  const suggestions = []
  const collections = req.payload.config.collections.filter((collection) => !isInternalCollectionSlug(collection.slug))
