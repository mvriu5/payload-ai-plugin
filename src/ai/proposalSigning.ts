...
const signPayload = (payload: string) => createHmac("sha256", getSigningSecret()).update(payload).digest("hex")

export const signAIActionProposal = <Proposal extends SignableProposal>(proposal: Proposal): Proposal & { _aiSignature: AIActionSignature } => {
...
export const verifyActionProposal = (proposal: SignableProposal) => {
