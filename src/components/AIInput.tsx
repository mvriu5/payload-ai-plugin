...
export const AIInput = () => {
    ...
    const [mentionRange, setMentionRange] = useState<null | { end: number, start: number }>(null)
  ...
  const filteredCollections = collections.filter((collection) => collection.slug.toLowerCase().includes(mentionQuery.toLowerCase()),)
  const mentionSuggestions = filteredCollections.length < 3 ? [...filteredCollections, ...documentSuggestions] : filteredCollections
  ...
  useEffect(() => {
      const trimmedQuery = mentionQuery.trim()
        ...
        const fetchDocumentSuggestions = async () => {
            const res = await fetch(
                formatAdminURL({apiRoute: config.routes.api, path: '/ai-mention-suggestions'}), {
                    body: JSON.stringify({
            ...
            const result = (await res.json()) as { suggestions?: CollectionMentionOption[] }
    ...
    }, [config.routes.api, filteredCollections.length, mentionQuery, mentionRange])
  ...
  const insertMention = (suggestion: CollectionMentionOption) => {
      ...
      if (!mentionRange || !editor) return
        ...
        const badgeText = suggestion.type === 'collection' ? `collection: ${suggestion.label}` : `document: ${suggestion.label}`
        const promptText = suggestion.type === 'collection' ? badgeText : `${badgeText} (${suggestion.collection}/${suggestion.id})`
    ...
    const handleSubmit = async () => {
        ...
        try {
            const res = await fetch(
                formatAdminURL({ apiRoute: config.routes.api, path: '/ai-chat' }), {
    ...
    const handleApplyProposal = async (proposal: AIActionProposal, index: number) => {
        setIsApplying(true)
        setError('')

        try {
            const res = await fetch(
                formatAdminURL({ apiRoute: config.routes.api, path: '/ai-apply-action' }), {
            ...
            if (!res.ok) throw new Error(result.error || 'Could not apply proposal')

            setAppliedProposalIndexes((currentIndexes) => [...currentIndexes, index,])
        } catch (err) {
